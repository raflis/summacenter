...
<div class="layoutContent">
    <div class="container-fluid profile">
        <div class="row show-header">
            <div class="col-sm-12">
                <h1>
                    <i class="fas fa-check-double fa-xs"></i> <span>Sliders Corretajes</span>
                </h1>
            </div>
        </div>
            ...
            <div class="col-sm-12 col-md-12">
                <div class="card shadow">
                    ...
                    <div class="texto pt-4">
                      @foreach ($home->corretaje_sliders as $item)
                        <div class="card-body">
                          @if ($loop->index >= 1)
                          ...
                          @endif
                          {!! Form::label('corretaje_sliders','Selecciona una imagen para desktop:',['class'=>'']) !!} <small><strong>(1440 x 473px)</strong></small> <code>*</code>
                          <div class="input-group">
                              ...
                              {!! Form::text('corretaje_sliders['.$loop->index.'][desktop]',$item['desktop'],['class'=>'form-control','id'=>'thumbnail'.$loop->iteration,'required']) !!}
                          </div>
                          <div id="holder{{ $loop->iteration }}" style="margin-top:15px;max-height:100px;">
                              <img src="{{ $item['desktop'] }}" alt="" style="height:5rem">
                          </div>

                          {!! Form::label('corretaje_sliders','Selecciona una imagen para mobile:',['class'=>'']) !!} <small><strong>(375 x 177px)</strong></small> <code>*</code>
                          <div class="input-group">
                              <span class="input-group-btn">
                              ...
                              {!! Form::text('corretaje_sliders['.$loop->index.'][mobile]',$item['mobile'],['class'=>'form-control','id'=>'thumbnail_'.$loop->iteration,'required']) !!}
                          </div>
                          <div id="holder_{{ $loop->iteration }}" style="margin-top:15px;max-height:100px;">
                              <img src="{{ $item['mobile'] }}" alt="" style="height:5rem">
                          </div>

                          {!! Form::label('corretaje_sliders','Link:',['class'=>'mt-3']) !!} <code>*</code>
                          {!! Form::text('corretaje_sliders['.$loop->index.'][link]',$item["link"],['class'=>'form-control','required']) !!}
                          {!! Form::label('corretaje_sliders','Orden:',['class'=>'mt-3']) !!} <code>*</code>
                          {!! Form::number('corretaje_sliders['.$loop->index.'][order]',$item["order"],['class'=>'form-control','required']) !!}
...
<script>
	$(document).ready(function(){
		...
		var i={{ count(($home->corretaje_sliders))+1 }};
		...
		$('.añadir').on('click',function(e){
			...
			var template='<div class=""><div class="card-body pt-0">' +
							'<a href="#" class="btn btn-danger btn-circle btn-sm float-right mb-2 eliminar">' +
								'<i class="fas fa-trash"></i>' +
							'</a>' +
                '<label for="corretaje_sliders" class="">Selecciona una imagen para desktop:</label> <small><strong>(1440 x 473px)</strong></small> <code>*</code>' +
                '<div class="input-group">' +
                  '<span class="input-group-btn">' +
                      ...
                      '</a>' +
                  '</span>' +
                  '<input class="form-control" id="thumbnail'+i+'" name="corretaje_sliders['+i+'][desktop]" type="text" required>' +
                '</div>' +
                '<div id="holder'+i+'" style="margin-top:15px;max-height:100px;"></div>' +

                '<label for="corretaje_sliders" class="">Selecciona una imagen para mobile:</label> <small><strong>(375 x 177px)</strong></small> <code>*</code>' +
                '<div class="input-group">' +
                  '<span class="input-group-btn">' +
                      ...
                      '</a>' +
                  '</span>' +
                  '<input class="form-control" id="thumbnail_'+i+'" name="corretaje_sliders['+i+'][mobile]" type="text" required>' +
                '</div>' +
                '<div id="holder_'+i+'" style="margin-top:15px;max-height:100px;"></div>' +

                '<label for="corretaje_sliders" class="mt-3">Link:</label> <code>*</code>' +
								'<input class="form-control" name="corretaje_sliders['+i+'][link]" type="text" required>' +
								'<label for="corretaje_sliders" class="mt-3">Orden:</label> <code>*</code>' +
								'<input class="form-control" name="corretaje_sliders['+i+'][order]" type="number" required>' +
							'<hr class="mx-0 mt-4 border-bottom-dark" style="border:1px solid;background:#000">' + 
						'</div></div>' +
