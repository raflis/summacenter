...
<div class="layoutContent">
    <div class="container-fluid profile">
        <div class="row show-header">
            <div class="col-sm-12">
                <h1>
                    <i class="fas fa-check-double fa-xs"></i> <span>Sliders Nosotros</span>
                </h1>
            </div>
        </div>
            ...
            <div class="col-sm-12 col-md-12">
                <div class="card shadow">
                    ...
                    <div class="texto pt-4">
                      @foreach ($aboutus->sliders as $item)
                        <div class="card-body">
                          @if ($loop->index >= 1)
                          ...
                          <div class="input-group">
                              ...
                              {!! Form::text('sliders['.$loop->index.'][desktop]',$item['desktop'],['class'=>'form-control','id'=>'thumbnail'.$loop->iteration,'required']) !!}
                          </div>
                          ...
                          <div class="input-group">
                              ...
                              {!! Form::text('sliders['.$loop->index.'][mobile]',$item['mobile'],['class'=>'form-control','id'=>'thumbnail_'.$loop->iteration,'required']) !!}
                          </div>
                          <div id="holder_{{ $loop->iteration }}" style="margin-top:15px;max-height:100px;">
                              <img src="{{ $item['mobile'] }}" alt="" style="height:5rem">
                          </div>

                          {!! Form::label('sliders','Link:',['class'=>'mt-3']) !!} <code>*</code>
                          {!! Form::text('sliders['.$loop->index.'][link]',$item["link"],['class'=>'form-control','required']) !!}
			...
			var template='<div class=""><div class="card-body pt-0">' +
                ...
                '<div class="input-group">' +
                  '<span class="input-group-btn">' +
                      ...
                      '</a>' +
                  '</span>' +
                  '<input class="form-control" id="thumbnail'+i+'" name="sliders['+i+'][desktop]" type="text" required>' +
                '</div>' +
                ...
                '<div class="input-group">' +
                  '<span class="input-group-btn">' +
                      ...
                      '</a>' +
                  '</span>' +
                  '<input class="form-control" id="thumbnail_'+i+'" name="sliders['+i+'][mobile]" type="text" required>' +
                '</div>' +
                '<div id="holder_'+i+'" style="margin-top:15px;max-height:100px;"></div>' +

                '<label for="sliders" class="mt-3">Link:</label> <code>*</code>' +
								'<input class="form-control" name="sliders['+i+'][link]" type="text" required>' +
								'<label for="sliders" class="mt-3">Orden:</label> <code>*</code>' +
								'<input class="form-control" name="sliders['+i+'][order]" type="number" required>' +
							'<hr class="mx-0 mt-4 border-bottom-dark" style="border:1px solid;background:#000">' + 
						'</div></div>' +
