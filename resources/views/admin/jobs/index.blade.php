                    ...
                    <table class="table tableD">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $item->work_modality }}</td>
                                <td>{{ $item->workload }}</td>
                                <td>{{ $item->email }}</td>
                                <td>{{ count($item->applicants) }}</td>
                                <td>{{ ($item->status == 1)?'Si':'No' }}</td>
                                <td>{{ Carbon\Carbon::parse($item->created_at)->format('d/m/Y H:i:s') }}</td>
                                <td>
                                    <div style="display: inline-flex">
                                        <a class="btn btn-info btn-sm mr-1 {{ (count($item->applicants) == 0)?'disabled':'' }}" href="{{ route('jobs.show', $item->id) }}">
                                            <i class="fas fa-user-friends pr-1"></i> Postulantes
                                        </a>
                                        <a class="btn btn-primary text-white btn-sm mr-1" href="{{ route('jobs.edit', $item->id) }}">
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer justify-content-center">
            ...
            {!! Form::open(['route' => ['jobs.destroy', ''], 'method' => 'DELETE', 'id' => 'form-eliminar']) !!}
