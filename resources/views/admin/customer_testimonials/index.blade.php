    ...
    <div class="container-fluid">
        <div class="row layout-header">
            <div class="col-sm-12 header-content">
                <h1>
                    <i class="fas fa-bullhorn fa-xs text-white2"></i> Testimonios de Clientes
                </h1>
                <span class="subtitle">
            ...
            <div class="col-sm-12">
                <div class="card shadow">
                    <div class="card-header">
                        ...
                        <a class="btn btn-success" href="{{ route('customerstestimonials.create') }}">
                            <span class="icon">
                ...
                <div class="tablaTotal">
                    <table class="table tableD">
                        ...
                        <tbody>
                            @foreach ($customerstestimonials as $testimonial)
                            <tr>
                                <td>{{ $customerstestimonials->firstItem() + $loop->index }}</td>
                                <td><img src="{{ $testimonial->image }}" alt="" height=60></td>
                                <td>{{ $testimonial->name }}</td>
                                <td>{{ $testimonial->acquired_project }}</td>
                                <td>{!! Str::limit((html_entity_decode($testimonial->description)),80) !!}</td>
                                <td>{{ $testimonial->order }}</td>
                                <td>
                                    <div style="display: inline-flex">
                                        <a class="btn btn-primary text-white btn-sm mr-1" href="{{ route('customerstestimonials.edit', $testimonial->id) }}">
                ...
                <div class="paginacionTotal d-flex justify-content-end">
                    {{ $customerstestimonials->withQueryString()->render() }}
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer justify-content-center">
            ...
            {!! Form::open(['route' => ['customerstestimonials.destroy', ''], 'method' => 'DELETE', 'id' => 'form-eliminar']) !!}
...
$('document').ready(function(){
    $('.btn-eliminar').click(function(e){
        ...
        var base = '{{ route('customerstestimonials.destroy', '') }}';
        var url = base + '/' +id;
