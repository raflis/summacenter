    ...
    <div class="container-fluid">
        <div class="row layout-header">
            <div class="col-sm-12 header-content">
                <h1>
                    <i class="fas fa-bullhorn fa-xs text-white2"></i> Equipo de trabajo - Docentes
                </h1>
                <span class="subtitle">
            ...
            <div class="col-sm-12">
                <div class="card shadow">
                    <div class="card-header">
                        ...
                        <a class="btn btn-success" href="{{ route('worker_teachers.create') }}">
                            <span class="icon">
                ...
                <div class="tablaTotal">
                    <table class="table tableD">
                        ...
                        <tbody>
                            @foreach ($worker_teachers as $item)
                            <tr>
                                <td>{{ $worker_teachers->firstItem() + $loop->index }}</td>
                                <td><img src="{{ $item->image }}" alt="" height=40></td>
                                <td>{{ $item->fullname }}</td>
                                <td>{{ $item->course_area->name }}</td>
                                <td>{{ $item->position }}</td>
                                <td>{{ $item->order }}</td>
                                <td>
                                    <div style="display: inline-flex">
                                        <a class="btn btn-primary text-white btn-sm mr-1" href="{{ route('worker_teachers.edit', $item->id) }}">
                ...
                <div class="paginacionTotal d-flex justify-content-end">
                    {{ $worker_teachers->withQueryString()->render() }}
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer justify-content-center">
            ...
            {!! Form::open(['route' => ['worker_teachers.destroy', ''], 'method' => 'DELETE', 'id' => 'form-eliminar']) !!}
...
$('document').ready(function(){
    $('.btn-eliminar').click(function(e){
        ...
        var base = '{{ route('worker_teachers.destroy', '') }}';
        var url = base + '/' +id;
