    ...
    success: function success(response) {
      if (response.success == true) {
        ...
        if (Object.keys(response.cart).length < 1) {
          list = '<div class="empty_cart">' + '<i class="fa-solid fa-cart-shopping"></i>' + '<br>' + 'Carrito vacío' + '</div>';
        } else {
          for (var i in response.cart) {
            html = '<div class="item" id="product_' + i + '">' + '<div class="image">' + '<img src="' + response.cart[i]['image'] + '" alt="">' + '</div>' + '<div class="item_detail">' + '<a class="h5_name" href="' + response.cart[i]['url'] + '">' + response.cart[i]['name'] + '</a>' + '<div delete_product_id=' + i + ' class="delete_product">' + '<i class="fa-solid fa-xmark"></i>' + '</div>' + '<p class="cat">Categoria: <a href="' + response.cart[i]['url_category'] + '" class="cat_name">' + response.cart[i]['category'] + '</a></p>' + '<div class="amount_product">' + 'Precio:' + '<div class="price_total">' + 'S/ <span class="price_total_product_' + i + '">' + response.cart[i]['price'] + '</span>' + '</div>' + '</div>' + '</div>' + '</div>';
            list = list + html;
          }
        }
...
$(function () {
  ...
  $('.burgergg').on('click', function () {
    ...
    if ($('.linea1').hasClass('toggle1')) {
      $('.linea1').removeClass('toggle1');
    } else {
      $('.linea1').addClass('toggle1');
    }

    if ($('.linea2').hasClass('toggle2')) {
      $('.linea2').removeClass('toggle2');
    } else {
      $('.linea2').addClass('toggle2');
    }

    if ($('.linea3').hasClass('toggle3')) {
